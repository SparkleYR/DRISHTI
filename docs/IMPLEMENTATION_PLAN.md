...
**Deliverables:** Local Moondream2 model files; typed `POST /api/v1/vlm/query` and `POST /api/v1/vlm/locate` contracts; explicit JPEG/base64 locator input or one replaceable in-memory snapshot from an active Walk session; one non-queueing VLM worker; timeout; CUDA free-memory guard; lazy per-request loading and immediate unload/cache release; a TTL-bounded detector landmark memory; optional per-frame device heading; memory-first target resolution with one-shot VLM fallback; session-scoped `IDLE`, `SEEKING`, `GUIDING`, `ARRIVED`, and `LOST` turn-by-turn guidance; relative bearing and coarse range telemetry; strict Risk Engine preemption; and a bounded latest-only target telemetry WebSocket. The VLM is never invoked by continuous Walk processing, no identity recognition is performed, and no frame is persisted. Android application work remains separate and unchanged in this backend pass.

**Acceptance criteria:** The query endpoint returns an accurate natural-language answer for a controlled image and prompt. A recently observed detector landmark resolves without loading the VLM; a memory miss performs exactly one current-frame VLM fallback. File/base64 and current-frame inputs validate against the typed contract. Heading-aware Walk frames produce turn-left/right, keep-turning, face-and-walk, arrived, and bounded reacquisition behavior; missing heading degrades to in-view guidance. Every non-`CLEAR` safety action suppresses target speech/haptics and exposes `is_safety_overridden=true`; a controlled critical obstacle still produces the existing immediate safety action. WebSocket subscribers receive newest-only target metadata without triggering inference. Landmark and frame memory are empty after session end and no frame is persisted. Runtime inference uses local files and CUDA only; timeout and concurrent-request behavior remain safe; insufficient VRAM degrades only the VLM request; and the detector/segmenter and Walk Mode remain available before, during, and after real fallback locator work without CUDA OOM. Phase 9 remains `IN_REVIEW` until automated checks, the amended real-model CUDA coexistence check, the controlled remembered-landmark/turn-by-turn demonstration, the coordinated Android contract update, and user gate approval pass.
