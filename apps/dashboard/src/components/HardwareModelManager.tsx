      ...
      <div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6 xl:grid-cols-4">
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 sm:col-span-2">
          <div className="flex items-start justify-between gap-4">
            <div className="flex gap-3"><span className="grid size-9 place-items-center rounded-lg bg-white text-blue-700 shadow-sm"><MemoryStick size={19} aria-hidden="true" /></span><div><p className="text-sm font-bold text-slate-950">RTX 4060 VRAM</p><p className="mt-0.5 text-xs text-slate-500">On-device CUDA memory</p></div></div>
          ...
          <div className="mt-5 h-3 overflow-hidden rounded-full bg-slate-200" role="progressbar" aria-label="RTX 4060 VRAM used" aria-valuemin={0} aria-valuemax={100} aria-valuenow={vram ? Math.round(usedPercent) : undefined}>
            <div className="h-full rounded-full bg-blue-700 transition-[width]" style={{ width: `${usedPercent}%` }} />
          </div>
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4">
      <div className="flex items-center justify-between gap-3">
        <span className="grid size-9 place-items-center rounded-lg bg-slate-100 text-slate-700"><Icon size={18} aria-hidden="true" /></span>
